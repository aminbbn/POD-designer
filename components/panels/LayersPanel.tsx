...
const LayersPanel: React.FC<LayersPanelProps> = ({
  ...
  return (
    <div className="space-y-3 animate-fade-in pb-10">
       ...
       {layers.length === 0 ? (
         <div className="flex flex-col items-center justify-center py-16 text-slate-600 border-2 border-dashed border-white/5 rounded-2xl bg-white/[0.02]">
                ...
                return (
                    <div 
                        ...
                        className={`
                            ...
                            ${dragOverIndex === index ? 'border-primary bg-primary/10 translate-y-1' : 'border-white/5 bg-white/5 hover:bg-white/[0.08] hover:border-white/10'}
                    ...
                    >
                        ...
                        {!isEditing && (
                            <div className="flex items-center gap-0.5 ml-2">
                                <button onClick={() => startEditing(index, layerName)} className="p-1.5 text-slate-500 hover:text-white hover:bg-white/10 rounded-md transition-colors opacity-0 group-hover:opacity-100">
                                    <Edit2 size={12} />
                                </button>
                                <div className="w-px h-3 bg-white/10 mx-1"></div>
                                <button onClick={() => onToggleVisibility(actualIndex)} className={`p-1.5 rounded-md transition-colors ${layer.visible !== false ? 'text-slate-400 hover:text-white hover:bg-white/10' : 'text-slate-600'}`}>
                                    {layer.visible !== false ? <Eye size={14} /> : <EyeOff size={14} />}
                                </button>
                                <button onClick={() => onToggleLock(actualIndex)} className={`p-1.5 rounded-md transition-colors ${layer.lockMovementX ? 'text-orange-400 bg-orange-400/10' : 'text-slate-400 hover:text-white hover:bg-white/10'}`}>
                                    <Lock size={14} />
                                </button>
