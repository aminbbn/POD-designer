...
const TopBar: React.FC<TopBarProps> = ({ onUndo, onRedo, onExport }) => {
  return (
    <div className="h-16 bg-surface border-b border-white/5 flex items-center justify-between px-6 z-30 shadow-sm relative">
      ...
      <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-1 bg-slate-900/50 p-1 rounded-lg border border-white/5 backdrop-blur-sm">
        ...
        */}
        <button onClick={onRedo} className="p-2 text-slate-400 hover:text-white hover:bg-white/10 rounded-md transition-colors" title="بازسازی">
          <Redo2 size={18} />
        </button>
        <div className="w-px h-4 bg-white/10" />
        <button onClick={onUndo} className="p-2 text-slate-400 hover:text-white hover:bg-white/10 rounded-md transition-colors" title="بازگشت">
          <Undo2 size={18} />
        </button>
      </div>
